...
public:
    int dots, source, target;
    std::ifstream fin;
 
    Graph() {
        fin.open("pathmgep.in");
        fin >> dots >> source >> target;
        --source;
        --target;
        table = new List[dots];
    }
    ...
    void Read_Data() {
        long long weight;
        for (int start = 1; start <= dots; ++start) {
            for (int end = 1; end <= dots; ++end) {
                fin >> weight;
                if (weight >= 0 && start != end) {
    ...
    long long Single_Shortest_Path(int end) {
        if (vertices[end].distance != pow(10, 15)) { return vertices[end].distance; }
        return -1;
    }
...
int main() {
    ...
    fout << graph.Single_Shortest_Path(graph.target);
}
